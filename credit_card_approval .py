...
sns.histplot(df["Income"], bins=10, kde=True, color="#1abc9c")  # Turquoise
...
sns.boxplot(x=df["Approval_Status"], y=df["Income"], color="pink")  # Purple
plt.title("Income vs. Approval Status", fontsize=14)
...
# Restore 'Demographics' column for fairness analysis
df["Demographics"] = pd.read_csv(csv_file)["Demographics"]  # Reload from original dataset

plt.figure(figsize=(8, 5))
sns.barplot(x=df["Demographics"], y=df["Predicted"], ci=None, color="0.3")  # Light Black (Gray)
plt.title("Fairness Analysis: Predictions by Demographics", fontsize=14)
